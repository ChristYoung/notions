(function() {
  ...
  DateRangePicker = (function() {
    function DateRangePicker($select) {
      ...
      this.isHidden = true;
      this.customOptionIndex = this.$select[0].length - 1;         //select里内容的索引值
      this.initBindings();
...
//    });
    ...
    DateRangePicker.prototype.formatDate = function(d) {
      return "" + (d.getFullYear().toString().substr(2, 2))+"年"+"/"+ (d.getMonth() + 1)+"月" + "/" + (d.getDate())+"日";
    };
  ...
  Calendar = (function() {
    ...
    Calendar.prototype.drawDateDisplay = function() {
      return this.$dateDisplay.text(this.year()+"年"+"/"+this.month()+"月"+"/"+this.day()+"日");
    };
