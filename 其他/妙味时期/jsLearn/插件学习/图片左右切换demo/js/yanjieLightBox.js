(function(exports){
    // 常用方法，原生js的实现
    var Util = {
    	//如果在初始化Slider对象不传参数的话,就是用默认的参数
        merge: function(){
            var obj = {};
            //将具有length属性的对象转换成数组,
            //现在这个变量args就含有一个含有函数所有参数的标准javascript数组对象
        ...
        wrapInner: function($el, className){
            ...
            $el.innerHTML = $inner.outerHTML; //outerHTML包裹标签名
        },
    ...
    var Slider = function(config){
        ...
        //this.config.el是必须要传的参数,如果this.config.el是一个字符串对象的话,就把它转换成一个dom对象
        this.$el = typeof this.config.el === 'string' ? Util.$(this.config.el) : this.config.el;
        this.itemNum = this.$el.childElementCount; //回传 element 子代 (child) 元素的数量
        this._init();
